    ...
    DriverTransferError<E>: From<E>,
{
    ...
    ///
    /// Read/write errors during communication with the `Driver` chip will also return an error.
...
#[cfg(feature = "async")]
#[allow(dead_code)]
impl<'a, I2C> Driver<Bus<'a, I2C>>
where
    I2C: embedded_hal::i2c::I2c<Error = Box<dyn embedded_hal::i2c::Error>>,
    // Bus<'a, I2C>: bus::BusTrait<Error = Box<dyn embedded_hal::i2c::Error>>,
{
    ...
    pub fn from_i2c(i2c: I2C, address: u8) -> Self {
        let bus = crate::bus::Bus::initialise(i2c, &address);
    ...
    pub fn write_bit(
        ...
        bit_addr: u8,
        bit_to_write: bool,
    ) -> Result<bool, Box<dyn embedded_hal::i2c::Error>> {
        let mut value = 0;
        if let Ok(reg_value) = self.bus.read_register_by_addr(reg_addr) {
            value = reg_value;
        }
        ...
        self.bus.write_register_by_addr(reg_addr, value).unwrap();
    ...
    /// Read seconds from the RTC clock
    #[allow(clippy::unnecessary_wraps)]
    pub fn read_seconds(&mut self) -> Result<u8, Box<dyn embedded_hal::i2c::Error>> {
        let secs = self.bus.read_register(Register::Seconds).unwrap();
    ...
    /// Read the year from the RTC clock
    #[allow(clippy::unnecessary_wraps)]
    pub fn read_year(&mut self) -> Result<u8, Box<dyn embedded_hal::i2c::Error>> {
        let year = self.bus.read_register(Register::Year).unwrap();

        Ok(bcd_to_dec(year))
    }

    /// Set the year and update the RTC clock
    // FIXME handel the unwrap with match.
    pub fn set_year(&mut self, year: u16) -> Result<u8, Box<dyn embedded_hal::i2c::Error>> {
        let year =
            dec_to_bcd(u8::try_from(year - 2000).expect("Unable to unwrap from TryFromIntError"));

        self.bus.write_register(Register::Year, year).unwrap();
